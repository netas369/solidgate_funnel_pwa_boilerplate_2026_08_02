...
- `public.sessions` is the single source of truth for current quiz state. One session is one row. Save the complete current answer snapshot in `sessions.quiz_answers`; never create a row for each answer.
...
- `public.funnel_events` is append-only milestone history. It is not the current state and must not contain full answer snapshots, email addresses, tokens, payment data, or unrestricted clickstream data.
