...
BEGIN
  ...
  ASSERT NOT EXISTS(SELECT 1 FROM public.orders WHERE auth_verified_at IS NOT NULL),
    'legacy email-based assignment or claimed_at is never backfilled as authentication proof';
  ASSERT NOT EXISTS (
  ...
  ASSERT NOT EXISTS (
    ...
    LEFT JOIN (SELECT 'orders' AS source,id,to_jsonb(o)-'auth_verified_at' AS row FROM public.orders o
