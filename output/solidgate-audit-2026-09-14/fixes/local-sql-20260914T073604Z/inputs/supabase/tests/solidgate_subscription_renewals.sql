...
-- The webhook records the paid invoice through PostgREST, which emits exactly
-- this shape: `ON CONFLICT (payment_environment, solidgate_invoice_id)` with no
-- index predicate. Postgres can only infer a NON-partial unique index for it.
-- Keep this the same statement PostgREST sends; do not add a WHERE clause to
-- make it pass.
DO $$
DECLARE
  v_count INTEGER;
BEGIN
  INSERT INTO public.renewal_events (
    ...
    solidgate_order_id, subscription_term_number, amount_cents,
    gross_amount_cents, refunded_amount_cents, status, currency
  )
  VALUES (
    'sandbox', 'inv-renewal-arbiter', 'sub-renewal-main',
    '71000000-0000-4000-8000-000000000101:trial1:2', 1, 500, 500, 0, 'paid', 'usd'
  )
  ON CONFLICT (payment_environment, solidgate_invoice_id) DO NOTHING;

  -- Redelivery of the same invoice must be a no-op, not a second ledger row.
  INSERT INTO public.renewal_events (
    payment_environment, solidgate_invoice_id, solidgate_subscription_id,
    solidgate_order_id, subscription_term_number, amount_cents,
    gross_amount_cents, refunded_amount_cents, status, currency
  ...
  SELECT COUNT(*) INTO v_count FROM public.renewal_events
  WHERE payment_environment = 'sandbox' AND solidgate_invoice_id = 'inv-renewal-arbiter';
  ASSERT v_count = 1, format('renewal ledger redelivery produced %s rows', v_count);

  RAISE NOTICE 'SOLIDGATE RENEWAL LEDGER ARBITER INDEX PASSED';
END $$;
