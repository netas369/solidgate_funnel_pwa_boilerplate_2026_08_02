...
BEGIN
  ...
  v_facts:=v_facts||'{"invoice_id":"finance-invoice-5","subscription_term_number":5,"quoted_amount_cents":1767}';
  PERFORM public.apply_solidgate_financial_event('sandbox','analytics-partial-capture','finance-analytics-order',
    v_facts||'{"captured_amount_cents":1200,"payment_status":"partial_settled"}',
    '{"event_key":"finance-analytics-partial","event_name":"payment_captured","distinct_id":"finance-user","insert_id":"81000000-0000-4000-8000-000000000891","properties":{"revenue":1200}}');
  PERFORM public.apply_solidgate_financial_event('sandbox','analytics-completed-capture','finance-analytics-order',
    v_facts||'{"captured_amount_cents":1767,"payment_status":"settle_ok"}',
    '{"event_key":"renewal:finance-invoice-5:paid","event_name":"subscription_renewed","distinct_id":"finance-user","insert_id":"81000000-0000-4000-8000-000000000892","properties":{}}');
  ASSERT (SELECT NOT properties?'revenue' AND (properties->>'captured_delta_cents')::INTEGER=1200
    AND (properties->>'net_delta_cents')::INTEGER=1200 AND (properties->>'captured_amount_cents')::INTEGER=1200
    FROM public.solidgate_analytics_outbox WHERE environment='sandbox' AND event_key='finance-analytics-partial'),
    'diagnostic capture retains amounts but cannot add duplicate conversion revenue';
  ASSERT (SELECT SUM((properties->>'revenue')::INTEGER)=1767 FROM public.solidgate_analytics_outbox
    WHERE environment='sandbox' AND event_key IN ('finance-analytics-partial','renewal:finance-invoice-5:paid')),
    'partial then canonical capture exposes 1767 cents of revenue, never 2967';
  ASSERT (SELECT SUM(captured_delta_cents)=1767 AND SUM(net_delta_cents)=1767 FROM public.solidgate_financial_movements
    WHERE environment='sandbox' AND solidgate_order_id='finance-analytics-order'), 'ledger capture total is independent from analytics event families';
