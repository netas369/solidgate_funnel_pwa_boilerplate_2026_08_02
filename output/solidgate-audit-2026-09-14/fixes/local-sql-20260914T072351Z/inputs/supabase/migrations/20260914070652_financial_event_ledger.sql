...
BEGIN
  ...
  v_new.refunded_amount_cents := GREATEST(v_old.refunded_amount_cents, COALESCE((p_facts->>'refunded_amount_cents')::INTEGER, 0));
  ...
  IF NULLIF(p_facts->>'chargeback_status','') IS NOT NULL THEN
    ...
    IF v_old.chargeback_occurred_at IS NULL OR v_chargeback_at > v_old.chargeback_occurred_at
       OR (v_chargeback_at = v_old.chargeback_occurred_at AND
          (v_reversed OR COALESCE(v_old.chargeback_status,'') NOT IN ('reversed','resolved_reversal'))) THEN
      v_new.chargeback_id := COALESCE(p_facts->>'chargeback_id', v_old.chargeback_id);
      v_new.chargeback_status := LOWER(p_facts->>'chargeback_status');
      v_new.chargeback_amount_cents := CASE WHEN v_reversed THEN 0 ELSE GREATEST(v_old.chargeback_amount_cents, COALESCE((p_facts->>'chargeback_amount_cents')::INTEGER, 0)) END;
