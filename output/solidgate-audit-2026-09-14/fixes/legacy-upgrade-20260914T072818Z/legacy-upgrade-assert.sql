...
BEGIN
  ...
  ASSERT (SELECT SUM(captured_delta_cents)=27600 AND SUM(refunded_delta_cents)=8500
    AND SUM(chargeback_delta_cents)=6900 AND SUM(net_delta_cents)=12200
  ...
  ASSERT (SELECT SUM((r->>'captured_cents')::BIGINT)=27600
    AND SUM((r->>'refunded_cents')::BIGINT)=8500
    AND SUM((r->>'chargeback_cents')::BIGINT)=6900
    AND SUM((r->>'net_cents')::BIGINT)=12200
    AND SUM((r->>'payments')::BIGINT)=8
    FROM jsonb_array_elements(v_report->'rows') r), 'report conserves opening balance amounts';
  ASSERT (SELECT SUM((r->>'net_cents')::BIGINT)=1600 AND SUM((r->>'payments')::BIGINT)=4
  ...
  END IF;
  RAISE NOTICE 'LEGACY POPULATED UPGRADE PASSED: 8 payments, captured 27600, refunded 8500, chargeback 6900, net 12200 cents; estimated provenance and idempotence verified';
END $$;
